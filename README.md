...
报告固定五段：**摘要 → 行业背景 → 商业模式 → 需求分析 → 术语表**，内嵌 5 类骨架图（发展时间轴、产品服务全景、业务协作链、资金流转、L1 流程）。可选一键同步到**飞书文档**（图一起传）。
...
```console
$ python3 scripts/check_report.py 城投定融行业分析报告.html
...
改了规则，就拿手头所有历史报告重跑一遍 `check_report.py`——规则改了而旧报告没跟上，要么该返修报告，要么该反省这条新规则是不是拍脑袋定的。
